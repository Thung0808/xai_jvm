 ...
 *     </ul>
 *   </li>
 *   <li>Measure Average Treatment Effect (ATE): E[Y | do(X=xâ‚)] - E[Y | do(X=xâ‚€)]</li>
 * </ol>
 * 
 * <h2>Usage Example</h2>
 * <pre>{@code
...
public class CausalExplainer {
        ...
        for (int i = 0; i < numFeatures; i++) {
            for (int j = 0; j < numFeatures; j++) {
                ...
                if (Math.abs(correlation) > 0.3) {
                    // Assume i â†’ j if i comes before j (temporal ordering heuristic)
                    if (i < j) {
                        graph.addEdge(i, j);
                    }
                }
    ...
    private int getFeatureIndex(String featureName) {
        // In production, maintain a feature name â†’ index mapping
        try {
    ...
    public record CausalEffect(
        ...
        public String confoundingInterpretation() {
            ...
            if (bias < 0.01) {
                return "Negligible confounding (correlation â‰ˆ causation)";
            } else if (bias < 0.05) {
                return "Moderate confounding detected";
            } else {
                return "Strong confounding detected (correlation â‰  causation)";
            }
        }
